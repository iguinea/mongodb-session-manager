...
1. **MongoDBSessionManager** (`mongodb_session_manager.py`): Main class extending `RepositorySessionManager` from Strands SDK
   - `sync_agent()`: Captures metrics via `agent.event_loop_metrics.get_summary()` including tokens, latency, TTFB, cycle metrics, tool usage, and writes them on the agent's last message — **except in the sync Strands runs for each `MessageAddedEvent`**, which fires before the event loop accumulates that cycle's metrics. `register_hooks()` wraps the registry with `sync_origin.MessageAddedTagging`, so those callbacks run with a `ContextVar` tag set. The closing sync (`AfterInvocationEvent`) and any explicit call still write them: a consumer writes the TTFT on the agent and syncs by hand (#66). `super().sync_agent()` runs in a `try/finally`: the agent state and this class' write are separate round-trips, and since #53 the second carries the messages of the turn
   - `append_message()`: what the event loop produces (`toolUse`, `toolResult`, the answer) waits for the write that closes the invocation and goes out in one `$push` with `$each`, with the metrics inside the last message's document. **The user's question does not wait** — it is the only thing in a turn nothing can produce again — and neither does a message appended outside an invocation, which has nothing bound to flush it: the manager brackets that window with its own `BeforeInvocationEvent`/`AfterInvocationEvent` callbacks. Safe because `AfterInvocationEvent` comes out of a `finally` in the SDK, so an invocation that raises still flushes. A failed batch is **not retried** (a write that raises may have been applied, and a retry would duplicate the turn); `close()` is the last chance for a batch whose invocation never closed. Reference turn: **8 → 4 writes**, and 2 instead of 2N+2 for an agent calling N tools (#53)
   - `delete_session()`: passthrough without arguments that drops `_pending_messages` first — a later `close()` would otherwise try to write them into a deleted session — and deletes the whole session document; the manager is spent afterwards. Not metadata-hooked: deleting the session is not a metadata operation. `FileSessionManager`/`S3SessionManager` raise `SessionException` here, but `delete_session` is not in the `SessionRepository` ABC of strands 1.56.0, so the house rule wins: `ValueError`, like every other write on a missing session (#132)
   - `get_metadata_tool()`: Returns Strands tool for agent metadata management. Its **docstring is the contract**: Strands builds the description and the input schema from it, and that is all the model reads. Do not pass `description=` or `inputSchema=` to the decorator again — the hand-written schema was assigned to `ToolSpec.inputSchema` without the `{"json": ...}` wrapper. It never broke an agent, because `ToolRegistry.validate_tool_spec()` wraps a bare schema before it reaches a provider (1.25 to 1.56); what it cost is that `normalize_schema()` invents the missing descriptions, so the model read `"Property action"` instead of what the docstring says. A key is a path in the three actions: `_handle_metadata_get()` resolves it with `field_names.resolve_path()` on the document the read already returned, and the reply of `set` names the keys whose value replaced a whole subdocument (#47)
   ...
   - `**kwargs` are MongoClient options as pymongo defines them (`pymongo.common.VALIDATORS`, case-insensitive), not a list of our own. **Nothing is dropped silently**: an unknown kwarg, client options with a borrowed `client=` (the factory path) and client options with an injected `session_repository=` each log a `WARNING` naming them. The Strands parent takes `**kwargs` and ignores them, so without this a typo vanished — which is also why the removed `metadataHook`/`feedbackHook` raise `TypeError` instead (#111)
...
2. **MongoDBSessionRepository** (`mongodb_session_repository.py`): Implements `SessionRepository` interface
   ...
   - `update_message_fields()` / `update_agent_fields()`: take keys relative to the message or the agent, and own the dot-notation paths. `update_agent_fields()` only writes when the agent already exists, so dotted `$set` cannot create a half-built agent (#119). The agent config rides along with the metrics in a single write
   - `create_messages()` appends a batch in one `$push: {$each: [...]}`, and `create_message()` is that method with a list of one. `fields_on_last` writes dot-notation keys on the last message of the batch, nested into the document with `field_names.apply_fields()` before the push — `$set` cannot reach a message the same write is creating, and that is how an invocation's metrics cost no write of their own. Command counts per operation are identical on MongoDB 8.2.7 and DocumentDB 5.0 (#53)
   - `delete_session()` removes the whole session document in one atomic `delete_one` — agents, messages, metadata, feedbacks and guardrail events share it. A missing session raises `ValueError` like every other write (`deleted_count` says so, no read first), and the repository forgets its in-memory knowledge of the session: the persisted-content cache is consulted before the store, so a stale one would turn the next sync of the same content into a silent no-op instead of `not found` (#132)
...
3. **MongoDBConnectionPool** (`mongodb_connection_pool.py`): Singleton for connection reuse
   ...
   - `initialize()` bounds its startup `ping` with `initialize_timeout_ms` (default 5000, `None` opts out): a startup SLA composed with the client's `timeoutMS` by `min` over positive values — `timeoutMS=0` is «no budget», not zero, and a looser explicit setting (`timeoutMS=60000`, a raised `serverSelectionTimeoutMS`) loses to the ceiling for this one ping. Without it the ping blocked startup for the accumulated phase budgets, ~35 s measured against a server that accepts TCP and goes mute (#122). The parameter never reaches `MongoClient` nor the singleton key, validation rejects anything but a positive int or `None` before any client is closed or built, and the ping's log names the deadline and its origin. Under an active CSOT deadline pymongo replaces the per-phase budgets with the time remaining — it does not clamp to them
...
5. **Hooks** (`hooks/`): Optional AWS integrations
   ...
   - A propagated delete carries the deleted keys as `null`, intersected with `metadata_fields` — with the fields selector the old Nones filter dropped every one of them and the consumer got `"metadata": {}` (#120). The selection is by **presence**: projecting the whole configuration (the fix the issue first proposed) would publish configured fields nobody deleted. In the WebSocket hook the delete's routing id is read from the projected document `get_metadata()` returns (`{"_id", "metadata"}`) — read flat it was always `None`, so a WebSocket delete **sent nothing at all**, with or without fields, and the test hid it by mocking the flat shape — and it is read **before** the delete write, winning over a tombstone of itself: deleting `connection_id` still delivers the event to the connection it just unbound
