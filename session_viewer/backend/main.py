...
def generate_error_response(
    status_code: int, error: Exception, context: str = "operation"
) -> tuple[str, str]:
...
async def lifespan(app: FastAPI):
    ...
    app.state.collection = factory._client[settings.database_name][
...
if settings.security_headers_enabled:
    ...
    logger.warning(
        "Security headers middleware DISABLED - not recommended for production"
    )
...
async def password_middleware(request: Request, call_next):
    ...
    excluded_paths = [
        "/health",
        "/api/session_viewer/v1/check_password",
        "/",
        "/components.js",
        "/viewer.js",
    ]
    ...
    if session_password_hash:
        ...
        if (
            ...
            and path_parts[2] == "session_viewer"
            and path_parts[3] == "v1"
            and path_parts[4] == "sessions"
        ):
            session_id = path_parts[5]

            is_valid, _used_global = validate_session_password(
                session_id, session_password_hash
            )
            if is_valid:
                logger.debug(
                    f"Session password authentication successful for {session_id}"
                )
                return await call_next(request)
            else:
                return JSONResponse(
                    status_code=401, content={"detail": "Invalid session password"}
                )
        else:
            return JSONResponse(
                status_code=403,
                content={
                    "detail": "Session password only grants access to specific session"
                },
            )
    ...
    if not password_hash:
        return JSONResponse(
            status_code=401, content={"detail": "Missing password header"}
        )
    ...
    try:
        ...
        if session_doc and "session_viewer_password" in session_doc:
            expected_hash = hashlib.sha256(
                session_doc["session_viewer_password"].encode()
            ).hexdigest()
            if password_hash == expected_hash:
        ...
        logger.error(
            f"Error validating session password for {truncate_session_id(session_id)}: {e}"
        )
        return (False, False)
...
def get_enum_values(
    collection: Any, field_name: str, max_values: int
) -> Optional[List[Any]]:
        ...
        for field_name in indexed_fields:
            ...
            if field_name in settings.enum_fields:
                ...
                values = get_enum_values(
                    collection, field_name, settings.enum_max_values
                )
            ...
            field_info = FieldInfo(field=field_name, type=field_type, values=values)
...
async def search_sessions(
    ...
    filters: Optional[str] = Query(
        None, description="JSON string with metadata filters"
    ),
    session_id: Optional[str] = Query(
        None, description="Session ID for partial matching"
    ),
    ...
    try:
        ...
        query = build_search_query(
            filters, session_id, created_at_start, created_at_end
        )

        logger.info(
            f"Searching sessions with query: {sanitize_query_for_logging(query)}"
        )
        ...
        for doc in documents:
            ...
            messages_count = sum(
                len(agent_data.get("messages", [])) for agent_data in agents.values()
            )
...
@app.get(
    "/api/session_viewer/v1/metadata-fields", response_model=MetadataFieldsResponse
)
...
async def health_check():
    ...
    try:
        ...
        return HealthResponse(status="unhealthy", mongodb="disconnected", error=str(e))


@app.post("/api/session_viewer/v1/sessions/{session_id}/check_password")
@limiter.limit(settings.rate_limit_auth)
async def check_session_password(
    ...
    try:
        ...
        if not password_hash:
            logger.warning(
                f"No password_hash in request for session {truncate_session_id(session_id)}"
            )
            return {"valid": False, "used_global": False}

        is_valid, used_global = validate_session_password(session_id, password_hash)

        return {"valid": is_valid, "used_global": used_global}

    except Exception as e:
        logger.error(
            f"Error checking session password for {truncate_session_id(session_id)}: {e}"
        )
        return {"valid": False, "used_global": False}
